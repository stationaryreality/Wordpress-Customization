...
$args = [
    ...
    'orderby'        => 'date',
    'order'          => $order,
    'post_parent__not_in' => [0], // ONLY attached images
];
...
<section class="admin-tool admin-tool-media-library">
    ...
    <header class="tool-header">
        ...
        <p>Browse all publicly attached images across the site.</p>
    ...
    <?php if ($media_query->have_posts()) : ?>
        ...
        <div class="media-library-grid">
            ...
            <?php while ($media_query->have_posts()) : $media_query->the_post();
                ...
                if (!$parent_post || $parent_post->post_status !== 'publish') continue;

                $thumb = wp_get_attachment_image($attachment_id, 'medium', false, ['loading' => 'lazy']);
                ...
                <article class="media-card">
                    <a href="<?php echo esc_url($full_url); ?>" target="_blank" class="media-thumb">
                        <?php echo $thumb; ?>
                    </a>

                    <div class="media-meta">
                        <div class="media-filename"><?php echo esc_html($filename); ?></div>

                        <?php if ($width && $height) : ?>
                            <div class="media-dimensions"><?php echo esc_html($width . ' × ' . $height); ?></div>
                        <?php endif; ?>

                        <div class="media-date">
                            Uploaded: <?php echo esc_html(get_the_date()); ?>
                        </div>
                        ...
                        <div class="media-parent-type">
                            Type: <?php echo esc_html(get_post_type($parent_id)); ?>
                        </div>
                    </div>
                </article>
