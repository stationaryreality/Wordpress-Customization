        ...
        <div class="taxonomy-review-inner">
            ...
            <div class="taxonomy-breakdown">
                <?php foreach ($type_counts as $type => $count): ?>
                    <?php $meta = get_cpt_metadata($type); ?>
                    <span>
            ...
            <p class="taxonomy-actions">
                <button type="button" class="select-all" data-term="<?php echo esc_attr($term->slug); ?>">
                    Select All
                </button>
                <button type="button" class="clear-all" data-term="<?php echo esc_attr($term->slug); ?>">
                    Clear
                </button>
            </p>
            ...
            <ul class="cpt-clean-list">
                <?php foreach ($posts as $post): ?>
                    ...
                    <li
                        ...
                        data-id="<?php echo intval($post->ID); ?>"
...
<section class="tool-tag-audit">
    ...
    <div class="tool-export-area">
        <button type="button" onclick="generateRemovalExport()">
            Generate Removal Export
        </button>
        <textarea id="removalExportOutput" class="removal-export-textarea" readonly placeholder="Grouped taxonomy removal IDs will appear here..."></textarea>
    </div>
    ...
    foreach ($themes as $theme) {
        render_taxonomy_review_term($theme, 'theme', $allowed_types);
    }
    ?>
    ...
    foreach ($topics as $topic) {
        render_taxonomy_review_term($topic, 'topic', $allowed_types);
    }
    ?>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    */
    ...
    document.querySelectorAll('.select-all, .clear-all').forEach(button => {
        button.addEventListener('click', () => {
            const slug = button.dataset.term;
            const isChecked = button.classList.contains('select-all');

            document
                .querySelectorAll(`[data-group="${slug}"] .review-checkbox`)
...
window.generateRemovalExport = function() {
    ...
    if (!checked.length) {
        document.getElementById('removalExportOutput').value = 'No items selected.';
        return;
    }
    ...
    Object.keys(grouped).sort().forEach(group => {
        ...
        Object.keys(grouped[group]).sort().forEach(type => {
            output += type + ':' + grouped[group][type].join(',') + "\n";
        });

        output += "\n-----------------\n\n";
    });

    document.getElementById('removalExportOutput').value = output.trim();
};
</script>